...
func TestGolden(t *testing.T) {
	...
	for _, g := range golden {
		...
		s = ChecksumFastIEEE([]byte(g.in))
		if s != g.ieee {
			t.Errorf("FastIEEE(%s) = 0x%x want 0x%x", g.in, s, g.ieee)
		}
...
func BenchmarkFastCrc32KB(b *testing.B) {
	b.SetBytes(1024)
	data := make([]byte, 1024)
	for i := range data {
		data[i] = byte(i)
	...
	for i := 0; i < b.N; i++ {
		h.Reset()
		//h.Write(data)
		ChecksumFastIEEE(data)
		h.Sum(in)
	}
}

func BenchmarkGzipCrc32KB(b *testing.B) {
	b.SetBytes(1024)
	data := make([]byte, 1024)
	for i := range data {
		data[i] = byte(i)
...
func BenchmarkCrcC32KB(b *testing.B) {
	castagnoliTab := MakeTable(Castagnoli)
	b.SetBytes(1024)
	data := make([]byte, 1024)
	for i := range data {
